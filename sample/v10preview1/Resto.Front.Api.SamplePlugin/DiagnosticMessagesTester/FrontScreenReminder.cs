        ...
        public FrontScreenReminder()
        {
            ...
            subscriptions.Add(PluginContext.Notifications.ScreenChanged
                .Subscribe(screen =>
                {
                    ...
                    else if (screen is IDeliveryClientEditScreen deliveryClientEditScreen)
                    {
                        var order = deliveryClientEditScreen.Order;
                        message = $"You are on the delivery order #{order.Number} client edit screen. Client's name: {order.Client?.Name ?? order.ClientName}.";
                    }
