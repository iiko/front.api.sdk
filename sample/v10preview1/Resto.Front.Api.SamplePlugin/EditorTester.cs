...
using Resto.Front.Api.Data.Documents;
...
namespace Resto.Front.Api.SamplePlugin
{
    internal sealed class EditorTester : IDisposable
    {
        ...
        private CheckBox chkDocumentInvoicePrinting;
        private TextBox couponsTextBox;
        private IDisposable documentInvoicePrintingSub;
        private static ConfigService Config => ServiceFactory.Instance.ConfigService;
        ...
        private void EntryPoint()
        {
            ...
            AddButton("Mark order as additional", MarkOrderAsAdditional);
            ...
            AddButton("Cancel unpaid and sent delivery", CancelUnpaidAndSentDelivery);
            ...
            AddButton("Update combo in order", UpdateOrderComboItems);
            AddButton("Decrease printed combo amount", DecreasePrintedComboAmount);
            ...
            AddButton("Complex update kitchen order external data", ComplexAddOrUpdateKitchenOrderAndItemsExternalData);
            AddButton("Complex delete kitchen order external data", ComplexDeleteKitchenOrderAndItemsExternalData);
            ...
            AddButton("Get past orders by external number", GetPastOrdersByExternalNumber);
            AddButton("Print past order cash memo cheque", PrintCashMemoCheque);
            AddButton("Get past order cash memo cheque formatted document", GetCashMemoMarkup);
            ...
            AddButton("Add group of products to stop list", AddProductsToStopList);
            AddButton("Set stop list product remaining amount", SetStopListProductRemainingAmount);
            AddButton("Set stop list products remaining amounts", SetStopListProductsRemainingAmounts);
            AddButton("Clear stop list", ClearStopList);
            AddButton("Remove product from stop list", RemoveProductFromStopList);
            AddButton("Remove group of products from stop list", RemoveProductsFromStopList);
            AddButton("Get stop list products remaining amounts", GetStopListProductsRemainingAmounts);
            AddButton("Change marking code (legacy)", ChangeMarkingCodeLegacy);
            AddButton("Change marking codes", ChangeMarkingCodes);
            AddButton("Skip scanning marking code", SkipScanningMarkingCode);
            ...
            AddButton("Set kitchen order status", SetKitchenOrderStatus);
            AddButton("Print on printing device", Print);
            AddButton("Log printing devices info", LogAllPrinterDevicesParameters);
            ...
            AddButton("Reset order counter", ResetOrderCounter);
            AddButton("Get changed orders", GetChangedOrders);
            AddDocumentInvoicePrintingChkBox();
            AddButton("AddCouponsToOrder",AddCouponsToOrder);
            AddCouponsTextBox();
            AddButton("RemoveAllCouponsFromOrder", RemoveAllCouponsFromOrder);
            window.ShowDialog();
        }

        private void AddCouponsTextBox()
        {
            var textBox = new TextBox()
            {
                Text = "Enter coupon numbers here, use (,) as separator",
                Margin = new Thickness(4)
            };

            buttons.Items.Add(textBox);
            couponsTextBox = textBox;
        }

        private void AddDocumentInvoicePrintingChkBox()
        {
            var chkBox = new CheckBox()
            {
                Content = "Add custom xml tags to document printed via document service",
                Margin = new Thickness(4)
            };

            buttons.Items.Add(chkBox);
            chkDocumentInvoicePrinting = chkBox;

            documentInvoicePrintingSub = PluginContext.Notifications.DocumentInvoicePrinting.Subscribe(AddCustomXmlToDocumentInvoice);
        }
            ...
            button.Click += (s, e) =>
            {
                ...
                catch (ConstraintViolationException ex)
                {
                    var message = $"{ex.GetType()}{Environment.NewLine}" +
                                  ...
                                  $"Reason: {ex.Reason}{Environment.NewLine}" +
                                  $"{ex.StackTrace}";
        ...
        /// <summary>
        /// Добавляет xml разметку к документу
        /// </summary>
        /// <param name="document"></param>
        /// <returns></returns>
        private ChequeExtensions AddCustomXmlToDocumentInvoice(IDocument document)
        {
            var docType = document.Type;
            ChequeExtensions result = null;

            chkDocumentInvoicePrinting.Dispatcher.Invoke(() =>
            {
                if (chkDocumentInvoicePrinting.IsChecked == true)
                {
                    result = new ChequeExtensions()
                    {
                        BeforeHeader = new XElement(Tags.Left, $"BeforeHeader doc type {docType}"),
                        AfterHeader = new XElement(Tags.Left, $"AfterHeader doc type {docType}"),
                        BeforeFooter = new XElement(Tags.Left, $"BeforeFooter doc type {docType}"),
                        AfterFooter = new XElement(Tags.Left, $"AfterFooter doc type {docType}")
                    };
                }
            });
        ...
        private static void MarkOrderAsAdditional()
        {
            ...
            switch (orders.Count)
            {
                case > 1:   // If you want to mark order as additional:
                    var additionalOrder = orders.Last();
                    var parentOrder = orders.First();
                    os.MarkOrderAsAdditional(additionalOrder, parentOrder, credentials);
                    break;
                case 1:     // If you want to clear order`s GroupOrderId:
                    var order = orders.Single();
                    os.MarkOrderAsAdditional(order, null, credentials);
                    break;
            }
        }
        ...
        private void CreateAndSendDelivery()
        {
            ...
            Debug.Assert(delivery.DeliveryStatus == DeliveryStatus.Confirmed);
        ...
        private void AddComboInOrder([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            ...
            {
                { new ComboGroupIdAndName(Guid.NewGuid(), "First step"), productItem },
                { new ComboGroupIdAndName(Guid.NewGuid(), "Second step"), compoundItem }
            };

            // идентификаторы связанных с комбо акции и программы лояльности, полученные из внешней системы лояльности (iikocard)
            // здесь заглушки в виде Guid.NewGuid() и Guid.Empty, поскольку интеграция с внешней системой лояльности выходит за рамки данного примера
            var (sourceActionId, programId) = (Guid.NewGuid(), Guid.Empty);
        ...
        /// <summary>
        /// Обновление состава комбо в заказе.
        /// </summary>
        private void UpdateOrderComboItems([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            if (order.Combos.Count <= 0 || order.Items.Count <= 0)
                return;

            var combo = order.Combos.First();
            var item = order.Items.OfType<IOrderProductItem>().First();
            os.UpdateOrderComboItems(combo.Id, new Dictionary<ComboGroupIdAndName, IOrderCookingItem>
                    { { new ComboGroupIdAndName(Guid.NewGuid(), "First step"), item } }, order, os.GetDefaultCredentials());
        }

        private static void DecreasePrintedComboAmount([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            var combo = order.Combos.Last();
            if (combo.Amount > 1)
            {
                var removalType = PluginContext.Operations.GetActiveRemovalTypes().Last(rt => rt.WriteoffType.Equals(WriteoffType.None));
                os.DecreasePrintedComboAmount(combo.Amount - 1, WriteoffOptions.WithoutWriteoff(removalType), combo, order, os.GetDefaultCredentials());
            }
        }
        ...
        {
            var externalDataItem = new ExternalDataItem("Sample plugin external data", false);
            os.AddOrderExternalData(PluginName, externalDataItem, order, os.GetDefaultCredentials());
        ...
        /// <summary>
        /// Отменить отправленную неоплаченную доставкy (только для режима - оплата при закрытии доставки).
        /// </summary>
        private void CancelUnpaidAndSentDelivery()
        {
            var order = PluginContext.Operations.GetDeliveryOrders().LastOrDefault();
            if (order == null)
                return;

            var credentials = PluginContext.Operations.GetDefaultCredentials();

            PluginContext.Operations.CancelSend(order, credentials);
            order = PluginContext.Operations.GetDeliveryOrderById(order.Id);

            var printedOrderItems = order.Items.Where(item => item.PrintTime != null && !item.Deleted).ToArray();
            if (printedOrderItems.Length > 0)
            {
                var removalType = PluginContext.Operations.GetActiveRemovalTypes().First();
                var writeoffOptions = WriteoffOptions.WithoutWriteoff(removalType);
                const string deleteItemComment = "Delivery cancelled.";

                PluginContext.Operations.DeletePrintedOrderItems(deleteItemComment, writeoffOptions, order, printedOrderItems, credentials);
            }
            var cancelCause = PluginContext.Operations.GetDeliveryCancelCauses().First();
            PluginContext.Operations.CancelNewDelivery(order, cancelCause, null, credentials);
        }
        ...
        {
            var hostTerminalsGroup = os.GetHostTerminalsGroup();
            var restaurantSection = os.GetTerminalsGroupRestaurantSections(hostTerminalsGroup).First();
            var tables = os.GetRestaurantSectionTables(restaurantSection);
            os.ChangeOrderTables(order, [tables.First()], os.GetDefaultCredentials());
        }
        ...
        private void AddPublicExternalData()
        {
            ...
            var externalDataItem = new ExternalDataItem("Sample plugin external data", true);
            PluginContext.Operations.AddOrderExternalData(PluginName, externalDataItem, order, PluginContext.Operations.GetDefaultCredentials());
        }
        ...
        {
            var kitchenOrder = PluginContext.Operations.GetKitchenOrders().Last();
            var externalDataItem = new ExternalDataItem("value", true);

            // ReSharper disable once ConditionIsAlwaysTrueOrFalse
            PluginContext.Operations.AddOrUpdateKitchenOrderExternalData(kitchenOrder, "key", externalDataItem);
        }
        ...
        private static void AddOrUpdateKitchenOrderItemExternalData()
        {
            ...
            // ReSharper disable once ConditionIsAlwaysTrueOrFalse
            PluginContext.Operations.AddOrUpdateKitchenOrderItemExternalData(kitchenOrder, baseOrderItemId, "item", externalDataItem);
        }
        ...
        private void ComplexAddOrUpdateKitchenOrderAndItemsExternalData()
        {
            const bool isPublic = true;
            var os = PluginContext.Operations;
            var kitchenOrder = os.GetKitchenOrders().Last();
            var orderExternalData = new Dictionary<string, ExternalDataItem>
            {
                { "OrderKey1", new("OrderValue1", !isPublic) },
                { "OrderKey2", new("OrderValue2", isPublic) }
            };

            var itemsExternalData = new Dictionary<IKitchenOrderCookingItem, IDictionary<string, ExternalDataItem>>();
            for (var i = 0; i < kitchenOrder.Items.Count; i++)
                itemsExternalData.Add(kitchenOrder.Items[i], new Dictionary<string, ExternalDataItem>
                    {
                        { "item" + (i + 1) + "Key1", new("item" + (i + 1) + "Value1", !isPublic) },
                        { "item" + (i + 1) + "Key2", new("item" + (i + 1) + "Value2", isPublic) }
                    });

            os.ComplexAddOrUpdateKitchenOrderAndItemsExternalData(kitchenOrder, orderExternalData, itemsExternalData);
        }

        private void ComplexDeleteKitchenOrderAndItemsExternalData()
        {
            var os = PluginContext.Operations;
            var kitchenOrder = os.GetKitchenOrders().Last();
            var orderExternalDataKeys = new HashSet<string> { "OrderKey1", "OrderKey2" };
            var itemsExternalDataKeys = new Dictionary<IKitchenOrderCookingItem, HashSet<string>>();
            for (var i = 0; i < kitchenOrder.Items.Count; i++)
                itemsExternalDataKeys.Add(kitchenOrder.Items[i],
                    new HashSet<string> { "item" + (i + 1) + "Key1", "item" + (i + 1) + "Key2" });

            os.ComplexDeleteKitchenOrderAndItemsExternalData(kitchenOrder, orderExternalDataKeys, itemsExternalDataKeys);
        }
        ...
        private void GetPastOrdersByExternalNumber()
        {
            var externalNumber = "ExternalNumber";
            var minCloseTime = new DateTime(2025, 3, 2);
            var maxCloseTime = new DateTime(2025, 3, 4);
            var isPartOfExternalNumber = true;
            var pastOrders = PluginContext.Operations.GetPastOrdersByExternalNumber(externalNumber, minCloseTime, maxCloseTime, isPartOfExternalNumber);
            var resultString = pastOrders.Aggregate(string.Empty, (current, order) => current + order.Number + Environment.NewLine);
            MessageBox.Show(resultString, "Past orders");
        }

        private void PrintCashMemoCheque()
        {
            var os = PluginContext.Operations;
            var pastOrder = os.GetPastOrders().Last();
            var printingDevice = os.GetPrintingDeviceInfos().Last();
            os.PrintCashMemoCheque(pastOrder, printingDevice, os.GetDefaultCredentials());
        }

        private void GetCashMemoMarkup()
        {
            var os = PluginContext.Operations;
            var pastOrder = os.GetPastOrders().Last();
            var printingDevice = os.GetPrintingDeviceInfos().Last();
            MessageBox.Show(os.GetCashMemoMarkup(pastOrder, printingDevice).Markup.Value);
        }
        ...
        private void AddProductsToStopList()
        {
            var stopListItems = new List<(IProduct, IProductSize)>();
            var activeProducts = PluginContext.Operations.GetActiveProducts()
                .Where(product => product.Template == null && product.Type == ProductType.Dish)
                .ToList();
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                stopListItems.Add((product, productSize));
            }

            PluginContext.Operations.AddProductsToStopList(stopListItems, PluginContext.Operations.GetDefaultCredentials());
        }
        ...
        [CanBeNull]
        private IOrderProductItem ChooseProduct([NotNull] IOrder order, [NotNull] IOperationService os, IViewManager vm)
        {
            ...
            return productItems[chooseResult];
        }

        private void ChangeMarkingCodeLegacy([NotNull] IOrder order, [NotNull] IOperationService os, IViewManager vm)
        {
            var productItem = ChooseProduct(order, os, vm);
            if (productItem == null)
                return;
            ...
            os.ChangeOrderItemMarkingCode(markingCode, order, productItem, os.GetDefaultCredentials());
        }

        private void ChangeMarkingCodes([NotNull] IOrder order, [NotNull] IOperationService os, IViewManager vm)
        {
            var productItem = ChooseProduct(order, os, vm);
            if (productItem == null)
                return;

            var result = vm.ShowInputDialog("Enter number of codes", InputDialogTypes.Number) as NumberInputDialogResult;
            if (result is not { Number: > 0 })
                return;

            // Add marking codes
            var codes = new List<OrderItemIdentifierCode>();

            for (int i = 0; i < result.Number; i++)
            {
                codes.Add(new OrderItemIdentifierCode
                {
                    Id = Guid.NewGuid(),
                    Code = new IdentifierCode { Value = $"code{i}" },
                    Flags = IdentifierCodeFlags.MarkingCode | IdentifierCodeFlags.SendToFiscal
                });
            }

            os.AddIdentifierCodesToOrderItemUnit(codes, order, productItem, os.GetDefaultCredentials());
            // Reload entities
            order = os.GetOrderById(order.Id);
            productItem = order.Items.First(item => item.Id == productItem.Id) as IOrderProductItem;


            // Set verification result
            var verificationResult = new CodeVerificationResult
            {
                IsAllowed = true,
            ...
            os.ChangeOrderItemVerificationResult(codes.First().Id, "Crpt", verificationResult, order, productItem,
                os.GetDefaultCredentials());
            // Reload entities
            order = os.GetOrderById(order.Id);
            productItem = order.Items.First(item => item.Id == productItem.Id) as IOrderProductItem;


            // Remove marking codes
            os.DeleteOrderItemIdentifierCodes([codes.Last().Id], order, productItem!, os.GetDefaultCredentials());

            // Log all marking codes
            var newOrder = os.GetOrderById(order.Id);
            var sb = new StringBuilder();
            sb.Append("Marking codes for products:");
            foreach (var orderItem in newOrder.Items.OfType<IOrderProductItem>())
            {
                sb.AppendLine($"Product: {orderItem.Product.Name} amount={orderItem.Amount}");
                var unitIndex = 0;
                if (orderItem.Codes == null)
                    continue;
                foreach (var codesPerUnit in orderItem.Codes)
                {
                    sb.AppendLine($"  Unit {++unitIndex}");
                    foreach (var markingCode in codesPerUnit)
                    {
                        sb.AppendLine($"    {markingCode.Code.Value} flags={markingCode.Flags}");
                        if (markingCode.VerificationResults != null)
                            foreach (var pair in markingCode.VerificationResults)
                            {
                                sb.AppendLine(
                                    $"      Verification result: source={pair.Key} isAllowed={pair.Value.IsAllowed}, message={pair.Value.UserMessage}");
                            }
                    }
                }
            }
        ...
        private void SkipScanningMarkingCode([NotNull] IOrder order, [NotNull] IOperationService os, IViewManager vm)
        {
            var orderItems = order.Items;
            var productItems = orderItems.OfType<IOrderProductItem>().Where(item => item.Product.OuterEconomicActivityNomenclatureCode?.AllowSkipMarkScan == true).ToList();
            if (!productItems.Any())
            {
                vm.ShowErrorPopup("No active products!");
                return;
            ...
            var chooseResult = vm.ShowChooserPopup("Select product to skip scanning marking code", productItems.Select(item => item.Product.Name).ToList());
            if (chooseResult < 0)
                return;

            os.SkipScanningOrderItemMarkingCode(order, productItems[chooseResult], os.GetDefaultCredentials());
        }
        ...
        private void RemoveProductsFromStopList()
        {
            var products = PluginContext.Operations.GetStopListProductsRemainingAmounts();
            if (products.Count == 0)
                return;

            var productsWithSizes = products.Select(product => (product.Key.Product, product.Key.ProductSize)).ToList();
            PluginContext.Operations.RemoveProductsFromStopList(productsWithSizes, PluginContext.Operations.GetDefaultCredentials());
        }
        ...
        private void SetStopListProductsRemainingAmounts()
        {
            var stopListItems = new List<(IProduct, IProductSize, decimal)>();
            var activeProducts = PluginContext.Operations.GetActiveProducts()
                .Where(product => product.Template == null && product.Type == ProductType.Dish)
                .ToList();
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                stopListItems.Add((product, productSize, amount));
            }

            PluginContext.Operations.SetStopListProductsRemainingAmounts(stopListItems, PluginContext.Operations.GetDefaultCredentials());
        }
        ...
        private void SetKitchenOrderStatus()
        {
            var order = PluginContext.Operations.GetKitchenOrders().Last();
            var currentStatus = order.ProcessingStatus;
            //If you want to set status manually:
            if (order.ProcessingStatus < KitchenOrderProcessingStatus.Served)
                PluginContext.Operations.SetKitchenOrderProcessingStatus(order, ++currentStatus);
            else
                PluginContext.Operations.SetKitchenOrderProcessingStatus(order, --currentStatus);

            //If you want to return the status setting to automatic mode:
            //PluginContext.Operations.SetKitchenOrderProcessingStatus(order);

            MessageBox.Show(PluginContext.Operations.GetKitchenOrderById(order.Id).ProcessingStatus.ToString());
        }
        ...
        private void LogAllPrinterDevicesParameters()
        {
            var printingDevices = PluginContext.Operations.GetPrintingDeviceInfos();
            foreach (var printingDevice in printingDevices)
            {
                var driverParams = PluginContext.Operations.GetPrinterDriverParameters(printingDevice);
                LogPrinterDevicesParameters(printingDevice.Id, printingDevice.FriendlyName, driverParams);
            }

            MessageBox.Show("Printer devices have been logged.");
        }

        private void LogPrinterDevicesParameters(Guid id, string friendlyName, Data.Device.Settings.PrinterDriverParameters driverParams)
        {
            var logInfo = $"Id={id}, Name={friendlyName}, " +
                          $"font0Width={driverParams.Font0Width}, " +
                          $"font1Width={driverParams.Font1Width}, " +
                          $"font2Width={driverParams.Font2Width}, " +
                          $"PageWidth={driverParams.PageWidth}, " +
                          $"PageHeight={driverParams.PageHeight}, " +
                          $"Dpi={driverParams.Dpi}, " +
                          $"MarginLeft={driverParams.MarginLeft}, " +
                          $"MarginRight={driverParams.MarginRight}, " +
                          $"MarginTop={driverParams.MarginTop}, " +
                          $"MarginBottom={driverParams.MarginBottom}, " +
                          $"canUseFontSizes={driverParams.CanUseFontSizes}, " +
                          $"canPrintBarcode={driverParams.CanPrintBarcode}, " +
                          $"canPrintLogo={driverParams.CanPrintLogo}, " +
                          $"canPrintQRCode={driverParams.CanPrintQRCode}, " +
                          $"CanPrintImage={driverParams.CanPrintImage}";
            if (driverParams is Data.Device.Settings.CashRegisterDriverParameters cashRegisterDriverParams)
                logInfo += $", CanPrintText={cashRegisterDriverParams.CanPrintText}, " +
                           $"zeroCashOnClose={cashRegisterDriverParams.ZeroCashOnClose}, " +
                           $"supportsPayInAfterSessionClose={cashRegisterDriverParams.SupportsPayInAfterSessionClose}, " +
                           $"isCancellationSupported={cashRegisterDriverParams.IsCancellationSupported}, " +
                           $"isBillTaskSupported={cashRegisterDriverParams.IsBillTaskSupported}, " +
                           $"needToCheckBillNumber={cashRegisterDriverParams.NeedToCheckBillNumber}, " +
                           $"isRegisterStatusSupported={cashRegisterDriverParams.IsRegisterStatusSupported}, " +
                           $"printNonFiscalPrepayCheque={cashRegisterDriverParams.PrintNonFiscalPrepayCheque}, " +
                           $"printDetailedChequeWithPrepay={cashRegisterDriverParams.PrintDetailedChequeWithPrepay}, " +
                           $"isCustomDuplicateCheckSupported={cashRegisterDriverParams.IsCustomDuplicateCheckSupported}, " +
                           $"IsBuyChequeSupported={cashRegisterDriverParams.IsBuyChequeSupported}, " +
                           $"IsMultipleMarkingCodesPerUnitSupported={cashRegisterDriverParams.IsMultipleMarkingCodesPerUnitSupported}";
            PluginContext.Log.Info(logInfo);
        }
        ...
        private void ResetOrderCounter()
        {
            PluginContext.Operations.ResetOrderCounter();
        }

        /// <summary>
        /// Get changed orders and update the max known revision of orders.
        /// If you need to monitor different types of objects, make a series requests from the same 'revisionFrom':
        /// <see cref = "IOperationService.GetChangedOrders"/>,
        /// <see cref = "IOperationService.GetChangedDeliveryOrders"/>,
        /// <see cref = "IOperationService.GetChangedKitchenOrders"/>,
        /// <see cref = "IOperationService.GetChangedReserves"/>.
        /// Select the maximum revision from the responses and use it as a 'revisionFrom' for the next requests.
        /// </summary>
        private void GetChangedOrders()
        {
            var orders = PluginContext.Operations.GetChangedOrders(Config.MaxKnownVersionedEntityRevision);
            if (orders.Entities.IsEmpty())
                return;

            Config.MaxKnownVersionedEntityRevision = orders.RevisionTo;
            Config.Save();
            PluginContext.Log.InfoFormat($"Max revision of orders = {orders.RevisionTo}.");

            var orderWithMaxRevision = orders.Entities.Single(x => x.Revision == orders.RevisionTo);
            MessageBox.Show(orderWithMaxRevision.Entity != null
                    ? $"The last edited order Id = {orderWithMaxRevision.Id} has revision {orders.RevisionTo}."
                    : $"The order Id = {orderWithMaxRevision.Id} with max revision = {orderWithMaxRevision.Revision} was deleted.",
                "SamplePlugin", MessageBoxButton.OK, MessageBoxImage.None);
        }

        private void AddCouponsToOrder([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            var textBoxString = couponsTextBox.Text;
            textBoxString = textBoxString.Replace(" ", "");

            // Разделяем по запятой и убираем пустые строки
            var couponsNumbers = textBoxString
                .Split([','], StringSplitOptions.RemoveEmptyEntries)
                .Where(s => !string.IsNullOrWhiteSpace(s))
                .ToList();

            if (couponsNumbers.Count == 0)
                return;

            os.ChangeOrderIikoCard5Coupons(couponsNumbers, order, os.GetDefaultCredentials());
        }

        private void RemoveAllCouponsFromOrder([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            os.ChangeOrderIikoCard5Coupons(null, order, os.GetDefaultCredentials());
        }

        public void Dispose()
        {
            documentInvoicePrintingSub?.Dispose();
