...
namespace Resto.Front.Api.SamplePlugin.NotificationHandlers
{
    internal sealed class NavigatingToPaymentScreenHandler : IDisposable
    {
        ...
        private void OnNavigatingToPaymentScreen([NotNull] IOrder order, [NotNull] IOperationService os, [NotNull] IViewManager vm, INavigatingToPaymentScreenOperationContext context)
        {
            ...
            var tables = os.GetRestaurantSectionTables(orderSection);
            var currentTableIndex = tables.ToList().IndexOf(order.Tables[0]);
            var selectedTableIndex = vm.ShowChooserPopup("Select new table before navigating to payment screen", tables.Select(u => u.Name).ToList(), currentTableIndex);
            if (selectedTableIndex >= 0)
            {
                var selectedTable = tables[selectedTableIndex];
                os.ChangeOrderTables(order, [selectedTable], os.GetDefaultCredentials());
            }
