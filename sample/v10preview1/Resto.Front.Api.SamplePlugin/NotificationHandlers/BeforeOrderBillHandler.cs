    ...
    public class BeforeOrderBillHandler : IDisposable
    {
        ...
        private void OnBeforeOrderBill([NotNull] IOrder order, [NotNull] IOperationService os, [CanBeNull] IViewManager vm)
        {
            ...
            if (vm != null)
            {
                ...
                var tables = os.GetRestaurantSectionTables(orderSection).ToList();
                var currentTableIndex = tables.IndexOf(order.Tables[0]);
                var selectedTableIndex = vm.ShowChooserPopup("Select new table before order bill", tables.Select(u => u.Name).ToList(), currentTableIndex);
