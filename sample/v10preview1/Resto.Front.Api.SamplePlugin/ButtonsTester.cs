    ...
    internal sealed class ButtonsTester : IDisposable
    {
        ...
        public ButtonsTester()
        {
            subscriptions = new CompositeDisposable
            {
                ...
                Operations.AddButtonToPluginsMenu("SamplePlugin: Auth by user", x => AuthenticateByUser(x.vm)),
                Operations.AddButtonToPluginsMenu("SamplePlugin: Show yes no cancel popup", x =>
                {
                    var result = x.vm.ShowYesNoCancelPopup("Sample plugin", "?");
                    x.vm.ShowOkPopup("Sample plugin", $"{(result == null ? "Cancel" : result.Value ? "Yes" : "No")}");
                }),

                Operations.AddButtonToClosedOrderScreen("SamplePlugin: Show ok popup", x => ShowOkPopupOnClosedOrderScreen(x.vm), TriangleIcon),
                Operations.AddButtonToPastOrderScreen("SamplePlugin: Show ok popup", x => ShowOkPopupOnClosedOrderScreen(x.vm), TriangleIcon),
                ...
                Operations.AddButtonToPastOrderScreen("Print past order Invoice", x =>
                {
                    var pastOrder = x.pastOrder;
                    return;
                }, TriangleIcon),
                ...
                Operations.AddButtonToDocumentsScreen("Sample button", x =>
                {
                    x.vm.ShowOkPopup("Sample plugin", $"Current document number is {x.document.Number}, type is {x.document.Type}");
                }, IikoIcon),
                Operations.AddButtonToWindowsServiceMenu("Lorem ipsum dolor sit amet", vm =>
                {
                    vm.ShowInputDialog("Dialogus", InputDialogTypes.Number, null, "sic", "non");
                    //vm.ShowCheckPermissionPopup("F_EM", true);
                    //vm.ShowCheckPermissionsPopup(new [] { "F_EM" }, true, PermissionsCheckMode.All);
                    var numbers = new List<string>();
                    for (var i = 0; i < 10; i++)
                        numbers.Add("number " + i);
                    var rShowChooserPopup = vm.ShowChooserPopup("Chooser Title", numbers, 2, ButtonWidth.Default, "Custom Close");
                    vm.ShowQuantityChangerPopup("Quantity Changer Title", "Hint", 4, 9, new List<(string name, int quantity, int minimalQuantity, int maximalQuantity)>()
                    {
                        ("Vkusno i tochka", 1, 1, 3),
                        ("Durger Ping", 2, 2, 9),
                    }, "OK", "Cancel");
                    vm.ShowExtendedInputDialog("Extended Input", "Hint", new ExtendedInputDialogSettings(){DefaultDecimalValue = 1, EnableBarcode = true});
                    vm.ShowOkPopup("Ok Title", "Message", "Ok ok");
                    vm.ShowClosePopup("Close Title", "Close Message", "Close close");
                    vm.ShowErrorPopup("Error Title");
                    var yesNoResult = vm.ShowYesNoPopup("Yes No Title", "Message", "yes", "no");
                    var okCancelResult = vm.ShowOkCancelPopup("Ok Cancel Title", "Message", "okk");
                    var retryCancelResult = vm.ShowRetryCancelPopup("Retry Cancel", "message");
                    var yesNoCancel = vm.ShowYesNoCancelPopup("Yes no cancel title", "Message", "yes", "No no", "cancel");
                    var retryIgnoreCancel = vm.ShowRetryIgnoreCancelPopup("Retry ignore cancel", "message");
                    var keyboardResult = vm.ShowKeyboard("Keyboard title", "start text", true, 100000, false, false, "Oks", "Cancels");
                    vm.ShowExtendedKeyboardDialog("Extended keyboard");
                    //vm.ShowDateNumpadPopup(DateTime.Today, "Date title");
                    //vm.ShowCalendarPopup(DateTime.Today, "Calendar popup", DateTime.Today, DateTime.Now);
                    //vm.ShowDateTimePopup(DateTime.Today, "Date time", DateTime.Today, DateTime.Now);

                    //vm.ChangeProgressBarMessage("test");
                    //vm.NavigateToOrderAfterOperation(null);
                }),
                Operations.AddButtonToWindowsServiceMenu("KDS: изменить статус терминала", vm =>
                {
                    vm.ShowInputDialog("KDS: изменить статус терминала dialog", InputDialogTypes.Number, null, "KDS: изменить статус терминала Да", "KDS: изменить статус терминала Нет");
                }),
                Operations.AddButtonToWindowsServiceMenu("KDS: сбросить все заказы", vm =>
                {
                    vm.ShowInputDialog("KDS: сбросить все заказы dialog", InputDialogTypes.Number, null, "KDS: сбросить все заказы Да", "KDS: сбросить все заказы Нет");
                }),
                Operations.AddButtonToWindowsServiceMenu("KDS: сбросить заказы терминала", vm =>
                {
                    vm.ShowInputDialog("KDS: сбросить заказы терминала dialog", InputDialogTypes.Number, null, "KDS: сбросить заказы терминала Да", "KDS: сбросить заказы терминала Нет");
                }),
                Operations.AddButtonToWindowsServiceMenu("KDS: сбросить заказ KDS: сбросить заказ KDS: сбросить заказ KDS: сбросить заказ KDS: сбросить заказ", (vm) =>
                {
                    vm.ShowInputDialog("KDS: сбросить заказ KDS: dialog", InputDialogTypes.Number, null, "KDS: сбросить заказ Да", "KDS: сбросить заказ Нет");
                }),
                Operations.AddButtonToWindowsServiceMenu("KDS: история заказов", vm =>
                {
                    vm.ShowInputDialog("KDS: история заказов dialog", InputDialogTypes.Number, null, "KDS: история заказов Да", "KDS: история заказов Нет");
                }),
                Operations.AddButtonToWindowsServiceMenu("Long long long long long long Long long long long long longLong long long long long longLong long long long long longLong long long long long longLong long long long long longLong long long long long longLong long long long long longLong long long long long longLong long long long long longLong long long long long long", vm =>
                {
                    vm.ShowInputDialog("Long long long long long dialog", InputDialogTypes.Number, null, "Long long long Да", "Long long long Нет");
                }),
            };

            try
            {
                authUserWithoutPinLicenseSubscription = Licensing.AcquireSlot(21057201, Config.ClientId);
            }
            catch (Exception)
            {
                // ignored
        ...
        private void AuthenticateByUser(IViewManager viewManager)
        {
            ...
            try
            {
                credentials = Operations.AuthenticateByUser(userToAuthenticate, Config.ClientId);
            }
            catch (Exception e)
            {
