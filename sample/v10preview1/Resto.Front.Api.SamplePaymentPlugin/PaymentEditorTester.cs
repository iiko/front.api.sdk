...
namespace Resto.Front.Api.SamplePaymentPlugin
{
    internal sealed class PaymentEditorTester : IDisposable
    {
        ...
        private void EntryPoint()
        {
            ...
            AddButton("Print fiscal cheque before payment order local", PrintFiscalChequeBeforePaymentOrderLocal);
            AddButton("Print fiscal cheque before payment order remote", PrintFiscalChequeBeforePaymentOrderRemote);
            ...
            AddButton("Add cash payment additional currency rate", AddCashPaymentAdditionalCurrencyRate);
        ...
        private void AddCardPayment()
        {
            ...
            var additionalData = new CardPaymentItemAdditionalData("123456");
        ...
        private void AddCardExternalProcessedPayment()
        {
            const bool isProcessed = true;
            var order = PluginContext.Operations.GetOrders().Last(o => o.Status == OrderStatus.New);
            var paymentType = PluginContext.Operations.GetPaymentTypes().First(x => x.Kind == PaymentTypeKind.Card && x.Name.ToUpper() == "DINERS");
            var additionalData = new CardPaymentItemAdditionalData("123456", "0987654321", "test card type");
            var credentials = PluginContext.Operations.GetDefaultCredentials();
            PluginContext.Operations.AddExternalPaymentItem(order.ResultSum / 2, isProcessed, additionalData, null, paymentType, order, credentials);
        }
        ...
        /// <summary>
        /// Печать фискального чека перед оплатой заказа, существующими в заказе платежами, локально.
        /// </summary>
        private void PrintFiscalChequeBeforePaymentOrderLocal()
        {
            var order = PluginContext.Operations.GetOrders().Last(o => o.Status == OrderStatus.New || o.Status == OrderStatus.Bill);
            var credentials = PluginContext.Operations.AuthenticateByPin("777");
            PluginContext.Operations.PrintFiscalChequeBeforePaymentOrder(order, true, credentials);
        }

        /// <summary>
        /// Печать фискального чека перед оплатой заказа, существующими в заказе платежами, на главном терминале.
        /// </summary>
        private void PrintFiscalChequeBeforePaymentOrderRemote()
        {
            var order = PluginContext.Operations.GetOrders().Last(o => o.Status == OrderStatus.New || o.Status == OrderStatus.Bill);
            var credentials = PluginContext.Operations.AuthenticateByPin("777");
            PluginContext.Operations.PrintFiscalChequeBeforePaymentOrder(order, false, credentials);
        }
        ...
        #region Cash payment additional currency

        private void AddCashPaymentAdditionalCurrencyRate()
        {
            var order = PluginContext.Operations.GetOrders().Last(o => o.Status == OrderStatus.New);
            var paymentType = PluginContext.Operations.GetPaymentTypes().First(x => x.Kind == PaymentTypeKind.Cash);
            var credentials = PluginContext.Operations.GetDefaultCredentials();
            var currency = PluginContext.Operations.GetCurrencyRates().Last();
            PluginContext.Operations.AddExternalPaymentItem(100, false, null, null, paymentType, order, credentials, currency.Key);
        }
