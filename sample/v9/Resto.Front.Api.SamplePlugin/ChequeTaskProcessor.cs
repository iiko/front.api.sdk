...
namespace Resto.Front.Api.SamplePlugin
{
    internal sealed class ChequeTaskProcessor : IChequeTaskProcessor
    {
        ...
        public BeforeDoChequeActionResult BeforeDoChequeAction(ChequeTask chequeTask, ICashRegisterInfo device, CashRegisterChequeExtensions chequeExtensions, IViewManager viewManager)
        {
            ...
            if (needChangeInfo)
            {
                ...
                return new BeforeDoChequeActionResult
                {
        ...
        public BeforeDoChequeActionResult BeforeDoFfd11CorrectionOnPaymentOrderAction(Ffd11CorrectionTask correctionTask, ICashRegisterInfo device,
            IViewManager viewManager)
        {
            PluginContext.Log.InfoFormat("Before cheque correction on cash register: {0} ({1})", device.FriendlyName, device.Id);
            return null;
        }

        public void AfterDoChequeAction(ChequeTask chequeTask, PostResult result, ICashRegisterInfo device, IOperationService os, IViewManager viewManager)
        {
            PluginContext.Log.InfoFormat("After do cheque on cash register: {0} ({1})", device.FriendlyName, device.Id);
            const bool needToAddExternalData = true;
            if (needToAddExternalData)
            {
                var order = os.GetOrderById(chequeTask.OrderId);
                os.AddOrderExternalData("ChequeTaskProcessor", new ExternalDataItem("AfterDoChequeAction", true), order, os.GetDefaultCredentials());
            }
        }
        ...
        public void AfterDoFfd11CorrectionOnPaymentOrderAction(Ffd11CorrectionTask correctionTask, PostResult result, ICashRegisterInfo device,
            IOperationService os, IViewManager viewManager)
        {
            PluginContext.Log.InfoFormat("After cheque correction on cash register: {0} ({1})", device.FriendlyName, device.Id);
            const bool needToAddExternalData = true;
            if (needToAddExternalData)
            {
                var order = os.GetOrderById(correctionTask.OrderId);
                os.AddOrderExternalData("ChequeTaskProcessor", new ExternalDataItem("AfterDoFfd11CorrectionOnPaymentOrderAction", true), order, os.GetDefaultCredentials());
            }
        }
